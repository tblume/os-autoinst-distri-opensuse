...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-14-MACHINE-ID run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-14-MACHINE-ID RUN: .* \[OK\]" /tmp/testsuite.log';
}
