...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-06-SELINUX run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-06-SELINUX RUN: .* \[OK\]" /tmp/testsuite.log';
}
