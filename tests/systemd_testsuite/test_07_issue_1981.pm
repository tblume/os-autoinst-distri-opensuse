...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-07-ISSUE-1981 run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-07-ISSUE-1981 RUN: .* \[OK\]" /tmp/testsuite.log';
}
